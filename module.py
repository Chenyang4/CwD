...
from cl_lite.deep_inversion import GenerativeInversion, ClassMeanVar
from cl_lite.head import DynamicSimpleHead
from cl_lite.mixin import FeatureHookMixin
from cl_lite.nn import freeze, RKDAngleLoss, NormLoss
...
class Module(FeatureHookMixin, FinetuningMixin, cl.Module):
    ...
    def __init__(
        ...
        inv_alpha_ce: float = 1.,
        inv_alpha_cb: float = 1.,
        inv_alpha_pr: float = 0.001,
        inv_alpha_rf: float = 50.0,
        inv_alpha_gem: float = 0.,
        bn_mmt: float = 0.9,
    ...
    def register_losses(self):
        ...
        self.register_loss(
            "norm",
            NormLoss(),
            ["centroids", "num_old_class"],
            self.hparams.lambda_norm,
        )
    ...
    def update_old_model(self):
        ...
        self.inversion = GenerativeInversion(
            ...
            alpha_ce=self.hparams.inv_alpha_ce,
            alpha_cb=self.hparams.inv_alpha_cb,
            alpha_pr=self.hparams.inv_alpha_pr,
            alpha_rf=self.hparams.inv_alpha_rf,
            alpha_gem=self.hparams.inv_alpha_gem,
            bn_mmt=self.hparams.bn_mmt,
            meanvar=self.mean_var,
            writer=self.writer
        )
    ...
    def setup(self, stage=None):
        ...
        self.writer = SummaryWriter(log_dir=os.path.join(self.path, 'task'+str(current_task)))
    ...
    def on_train_end(self):
        super().on_train_end()
        self.mean_var = ClassMeanVar(
            self.datamodule,
            deepcopy(nn.Sequential(OrderedDict([("backbone", self.backbone), ("head", self.head)]))),
            self.inversion)
        print('memory size:', self.mean_var.class_mean.size(), '+', self.mean_var.covariance.size())
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.model_old is not None:
            ...
            kwargs["centroids"] = self.head.embeddings
            kwargs["num_old_class"] = self.model_old.head.num_classes
            assert kwargs["centroids"].requires_grad is True
    ...
    def validation_epoch_end(self, validation_step_outputs):
        ...
        self.writer.add_scalar('classifier/0.acc', self.evaluator.accuracy.compute().item(), self.current_epoch)
        if self.datamodule.current_task > 0:
            self.writer.add_scalar("classifier/1.acc_old", self.evaluator.accuracy_old.compute().item(), self.current_epoch)
            self.writer.add_scalar("classifier/2.acc_new", self.evaluator.accuracy_new.compute().item(), self.current_epoch)
